...
from experiments.SVS_Code.utils import build_env_by_name, from_env_name_to_class, from_env_name_to_class_test
from experiments.learning import shared_constants
from gym_pybullet_drones.utils.enums import DroneModel, Physics
from gym_pybullet_drones.envs.multi_agent_rl.FlockAviary import FlockAviary
from gym_pybullet_drones.envs.multi_agent_rl.LeaderFollowerAviary import LeaderFollowerAviary
#from gym_pybullet_drones.envs.multi_agent_rl.test_simpler_env.ReachThePointAviarySingle import ReachThePointAviarySingle
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_drones', default=1, type=int, help='Number of drones (default: 4)', metavar='')
    parser.add_argument('--env', default='ReachThePointAviarySingle', type=str, choices=['ReachThePointAviarySingle'],
                        help='Task (default: leaderfollower)', metavar='')
    parser.add_argument('--obs', default='aaa', type=ObservationType, help='Observation space (default: kin)',
                        metavar='')
    ...
    if ARGS.obs == ObservationType.KIN:
        OWN_OBS_VEC_SIZE = 12
    elif ARGS.obs == ObservationType.AAA:
        OWN_OBS_VEC_SIZE = 42
    ...
    rand = np.array([[-10, 0, 5]])

    if not ARGS.exp:
        env_callable, obs_space, act_space, temp_env = build_env_by_name(env_class=from_env_name_to_class_test(ARGS.env),
                                                                         exp=False,
...
#                "pol3": (None, obs_space[3], act_space[3], {"agent_id": 3, }),
    ...
    stop = {
        "timesteps_total": 3000000,  # 100000 ~= 10'
        #"episode_reward_mean": 240,
    ...
    if not ARGS.exp:
        ...
        OBS = ObservationType.KIN if ARGS.exp.split("-")[4] == 'kin' else ObservationType.AAA
        ...
        if ACT in [ActionType.ONE_D_RPM, ActionType.ONE_D_DYN, ActionType.ONE_D_PID]:
            ...
            exit()
        start = time.time()
        for i in range(360 * int(temp_env.SIM_FREQ / temp_env.AGGR_PHY_STEPS)):  # Up to 6''
            #### Deploy the policies ###################################
            temp = {}
            ...
            temp_env.render()
            if OBS == ObservationType.KIN or OBS == ObservationType.AAA:
                for j in range(NUM_DRONES):
                    logger.log(drone=j,
                               timestamp=i / temp_env.SIM_FREQ,
                               state=np.hstack([obs[j][0:3], np.zeros(4), obs[j][3:15], np.resize(action[j], (3))]),
                               control=np.zeros(12)
                               )                    
            sync(np.floor(i * temp_env.AGGR_PHY_STEPS), start, temp_env.TIMESTEP)
            #if done["__all__"]: break #obs = tes_env.reset() # OPTIONAL EPISODE HALT
        temp_env.close()
