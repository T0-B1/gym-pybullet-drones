...
SPHERE_IDS = []
ENV_COUNTER = 100 #VARIABILE OGNI QUANTI STEP VIENE RICREATO IL SET DI SFERE

cached_spheres = []
distances = np.array([])
reload_env_counter = ENV_COUNTER
env_number =  random.randint(1,10) #1
passedSphere = 0
...
class ReachThePointAviarySingle(BaseMultiagentAviary):
    ...
    def reset(self):
        ...
        if reload_env_counter > 0:
            ...
            reload_env_counter = ENV_COUNTER
            cached_spheres = []
            env_number = random.randint(1,10) #(env_number + 1) % 10
            print("CHANGING ENV")
            print(env_number)
    ...
    def _computeReward(self):
        ...
        for droneIndex in range(0, self.NUM_DRONES):
            ...
            if states[droneIndex, 0] < -20 or states[droneIndex, 1] < -10 or states[droneIndex, 1] > 10 or states[droneIndex, 2] > 10:
                    rewards[droneIndex] = -1#-5

            #da riattivare per 1.31.29 1.32.35 1.33.54
            #if states[droneIndex, 0] > 60:
            #    rewards[droneIndex] = 20
    ...
    def _computeDone(self):
        ...
        bool_val = True if self.step_counter / self.SIM_FREQ > 180 else False
    ...
    def _clipAndNormalizeState(self,
        ...
        normalized_pos_x = (clipped_pos_x-MIN_X) / (MAX_X -MIN_X) #self._normalize(clipped_pos_x, MIN_X, MAX_X)
